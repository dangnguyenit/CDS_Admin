...
class User < ActiveRecord::Base
  ...
  def self.get_all_user(page, per_page, search, sort = nil)
    ...
    if sort == "name"
      ...
      users = User.select("id, full_name, email, staff_number, status, career_path, created_at, team_leader_id, is_manager").order(sort).search_full_name(search).paginate(:page => page, :per_page => per_page)
    end
    ...
    users.each do |user|
      ...
      a[:role] = user.user_groups[0].name if user.user_groups[0]
  ...
  def self.get_user_to_teamlead(id, page, per_page, search, sort = nil)
    ...
    if sort.include?"name"
      users = thisDepartment.users.get_user_with_no_manager(manager_id).complex_sorting(sort).search_full_name(search).paginate(:page => page, :per_page => per_page)
    else  
      users = thisDepartment.users.get_user_with_no_manager(manager_id).order(sort).search_full_name(search).paginate(:page => page, :per_page => per_page)
    end
