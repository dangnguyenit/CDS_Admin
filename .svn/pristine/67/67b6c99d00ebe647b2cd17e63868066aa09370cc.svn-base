...
class UsersController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      if @user.is_manager
        array = Department.where(manager_id: @user.id).map(&:name)
        department_name = ""
        array.each{|i| department_name += i +", "}

        format.text { render text: "#{@user.full_name} currently is a manager, please remove he/she outside Project(s): #{department_name}", status: :unprocessable_entity }
      elsif @user.destroy
        format.json { render json: @user }
      end
    end
  ...
  def actions
    ...
    if message.eql?"assign"
      respond_to do |format|
        ...
        @teamleader_user = User.find(teamleader_id)
        main_department = Department.find(@user.main_department_id)
        array = @teamleader_user.departments.map(&:id)

        if array.include?(main_department.id)
          if @user.update_attributes(team_leader_id: teamleader_id) && @teamleader_user.update_attributes(is_team_leader: true)
            format.json { render json: @user }
          else
            format.json { render json: @user.errors, status: :unprocessable_entity }
          end
        else
          format.text { render text: "This Team Leader currently not in User's Main Project / Department, please active this Project / Departmet to main for user" }
        end
      end
    elsif message.eql?"remove"
      respond_to do |format|
        @teamleader_user = User.find(@user.team_leader_id)
        array_department_id = Department.all.map(&:id)
        array_teamleader_id = User.joins(:users_departments).where(["department_id IN (?)", array_department_id]).map(&:team_leader_id)
        
        if @user.update_attributes(team_leader_id: Department.find(department_id).manager_id)
          @teamleader_user.update_attributes(is_team_leader: false) unless array_teamleader_id.include?(@teamleader_user.id)
          format.json { render json: @user }
        else
          format.json { render json: @user.errors, status: :unprocessable_entity }
        end
      end
    elsif message.eql?("set_main_department")
      respond_to do |format|
        if @user.update_attributes(main_department_id: department_id.to_i)
          format.json { render json: @user}
        else
          format.json { render json: @user.errors, status: :unprocessable_entity }
        end
