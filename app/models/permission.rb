...
class Permission < ActiveRecord::Base
  ...
  def self.get_all_permissions(page, per_page, search, sort = nil,organization_id)
    ...
    permissions.each do |permission|
      ...
      unless a[:name].eql?("Manage Users PA")
        ...
        group_permission = Permission.where(group_permission_name: permission.group_permission_name).first
        a[:full] = group_permission.id
      end
