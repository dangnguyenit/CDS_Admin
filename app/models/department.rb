class Department < ActiveRecord::Base
  ...
  scope :complex_sorting_manager, lambda { |sort| includes(:users).order(sort)}
  ...
  def self.get_all_departments(page, per_page, search, sort = nil)
    ...
    if sort.include?"manager"
    	departments = Department.complex_sorting_manager("users.full_name " + sort.split(" ")[1]).search_name(search).paginate(:page => page, :per_page => per_page)
    ...
    departments.each do |d|
      ...
      a[:manager] = User.find(d.manager_id).full_name
