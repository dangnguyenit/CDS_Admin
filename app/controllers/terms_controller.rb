  ...
  SORT_MAP = {
	...
	def create
		...
		respond_to do |format|
			if @term.save
				# # Create Instances Terms and Instance for all user
				# User.where("id not in (?)", "1").each do |u|
			 #    new_instance = Instance.create(cds_template_id: u.departments[0].cds_template_id, user_id: u.id, status: "Active")
			 #    new_instance_term = InstancesTerm.create(term_id: term.id, instance_id: new_instance.id, attitude: "Good")
			 #  end
